...
server.post("/paciente/cadastrar", async (req, reply) => {
    ...
    // Transformando db.get em Promise
    const checkPaciente = () => {
        return new Promise((resolve, reject) => {
            const sqlCheck = `
            ...
            db.get(sqlCheck, [email, documento], (err, row) => {
                if (err) reject(err);
                else resolve(row);
            });
        });
    ...
    // Transformando db.run em Promise
    const insertPaciente = (senhaHash) => {
        return new Promise((resolve, reject) => {
            const sqlInsert = `
    ...
    try {
        ...
        const pacienteId = await insertPaciente(senhaHash);
...
server.get("/paciente/:id", (req, reply) => {
    const { id } = req.params;
    ...
    db.get(sql, [id], (err, row) => {
        if (err) {
            ...
            return reply.send({
                success: false,
                message: "Erro interno ao consultar o paciente."
            });
        }
...
// PUT /paciente/:id  — atualiza email, telefone e data_nascimento
server.put("/paciente/:id", async (req, reply) => {
  const { id } = req.params;
  const { email, telefone, data_nascimento } = req.body ?? {};

  console.log(`[PUT /paciente/${id}] body:`, req.body);

  if (!email && !telefone && !data_nascimento) {
    console.log("[PUT] Nenhum campo enviado");
    return reply.code(400).send({ success: false, message: "Nenhum campo enviado para atualização." });
  }
  ...
  try {
    ...
    const result = await runAsync(sqlUpdate, [
      email ?? null,
      telefone ?? null,
    ...
    console.log(`[PUT /paciente/${id}] run result:`, {
      lastID: result.lastID,
      changes: result.changes // sqlite3 sets this.changes
    });

    const pacienteAtualizado = await getAsync(
      `SELECT id, nome, documento, data_nascimento, telefone, email
       FROM paciente
       WHERE id = ?
       LIMIT 1`,
      [id]
    ...
    if (!pacienteAtualizado) {
      console.log(`[PUT /paciente/${id}] paciente não encontrado após update`);
      return reply.code(404).send({ success: false, message: "Paciente não encontrado após atualização." });
    }

    console.log(`[PUT /paciente/${id}] pacienteAtualizado:`, pacienteAtualizado);
    return reply.code(200).send(pacienteAtualizado);

  } catch (err) {
    console.error("❌ Erro PUT /paciente/:id:", err);
    return reply.code(500).send({ success: false, message: "Erro interno ao atualizar paciente." });
  }
});

server.delete("/paciente/:id", async (req, reply) => {
    const { id } = req.params;
    ...
    try {
        ...
        const result = await new Promise((resolve, reject) => {
            db.run(sql, [id], function (err) {
                if (err) return reject(err);
                resolve(this.changes); // quantas linhas foram afetadas
            });
        });
        ...
        return reply.code(500).send({
            success: false,
            message: "Erro interno ao excluir paciente."
        });
    }
});
